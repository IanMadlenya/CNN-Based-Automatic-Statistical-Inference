...
def Net(model_type):
    ...
    n.conv2 = L.Convolution(n.pool1, param=w_lr, kernel_size=kerSize, pad=padding, stride=1,num_output=oo2, weight_filler=dict(type=g, std=0.01), bias_filler=dict(type=c))
    n.relu2 =        L.ReLU(n.conv2, in_place=True)

    n.conv3 = L.Convolution(n.relu2, param=w_lr, kernel_size=kerSize, pad=padding, stride=1,num_output=oo2, weight_filler=dict(type=g, std=0.01), bias_filler=dict(type=c))
    n.relu3 =        L.ReLU(n.conv3, in_place=True)
    
    n.pool3 =     L.Pooling(n.relu3, kernel_size=3, stride=2, pool=P.Pooling.AVE)
    
    n.ip1 =  L.InnerProduct(n.pool3, param=w_lr, num_output=oo3, weight_filler=dict(type='gaussian',  std=0.1), bias_filler=dict(type='constant'))
